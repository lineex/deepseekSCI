...
2. **目标数据库全浏览器操控路线图 (Full Browser Operation Roadmap)**：
   ...
   - 🌐 **Cochrane Library** (`https://www.cochranelibrary.com`)：通过 `browser_*` 打开搜索页，自动等待 DOM 节点 `#searchResults` 渲染完成（解决 0 条目问题），分别提取 Reviews, Protocols 与 CENTRAL 条目及 CD 号/DOI。
   - 🌐 **Europe PMC** (`https://europepmc.org`)：通过 `browser_*` 打开页面，自动等待 JS 节点完成渲染，提取 PMCID、Open Access PDF 下载按钮与全文 XML。
   - 🌐 **Web of Science** (`https://www.webofscience.com`)：通过 `browser_*` 导航至 WoS，继承机构 EZproxy/WebVPN 认证，输入 `TS=(...)` 表达式，逐页提取 WoS Accession ID、Times Cited 与 DOI。
   - 🌐 **Embase** (`https://www.embase.com`)：结合 `embase-session` / `embase-web-search` 与 `browser_*` 自动操控导航并提交 Emtree 表达式，抓取文献卡片。
   - 🌐 **Scopus** (`https://www.scopus.com`)：通过 `browser_*` 打开 Scopus 检索页，提取 TITLE-ABS-KEY 结果及 EID/DOI。
   - 🌐 **Google Scholar** (`https://scholar.google.com`)：通过 `browser_*` 模拟真实人类操作（带 3-5 秒 Jitter 停顿），输入关键词检索，提取引用次数与 PDF 直链；遇到 429 时自动增加退避延迟。
...
- **Tier 3 (网络 HTTP 异常/API 限流/429/JS依赖)**：自动启动指数退避 (Exponential Backoff) 重试，或全面切换至 `browser_*` 浏览器操控完成 DOM 解析与数据提取。
...
1. **全浏览器操控精准多库联合检索**：使用 `browser_*` 顺序检索 PubMed, Embase, Web of Science, Cochrane Library, Scopus, Europe PMC, OpenAlex, Google Scholar。
2. **自动去重与 PRISMA 2020**：运行 Python 自动去重脚本（处理 DOI 与 Title 匹配），输出分库检索量与去重后总量的 PRISMA 流程图数据表。
3. **100% 全文提取与偏倚评价**：通过浏览器直接操控或 API 深入全文提取样本量、干预措施、结局指标及调整后 HR/OR/RR (95% CI)，完成 ROB2 / ROBINS-I 测评。
