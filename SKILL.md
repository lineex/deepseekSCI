...
## 📄 强制 100% 全数据库摘要全量提取与 PICO 解析 Protocol (Mandatory All-DB Abstract & Full-Text Deep Extraction Protocol)

> [!IMPORTANT]
> **彻底解决所有数据库“只检索标题而不提取摘要”问题的核心强制规范：**

1. **12 大目标数据库全覆盖指令 (100% All-Database Mandatory Directive)**：
   - 🚫 **无一例外严禁**在任何数据库中仅列出标题 (Title)、PMID 或 DOI 即宣布检索完成！
   - ✅ **强制要求**：无论是 PubMed, Embase, WoS, Cochrane, Scopus, Google Scholar, Europe PMC, OpenAlex, ClinicalTrials.gov, IEEE Xplore, ScienceDirect 还是 arXiv/bioRxiv，**全部 12 个数据库必须 100% 抓取每篇文献的完整摘要 (Full Abstract)**、发表年份、期刊名称、作者及其机构。

2. **摘要与全文 PICO 结构化解析流水线 (Structured Abstract & PICO Parsing Pipeline)**：
   - 从全部数据库提取摘要后，必须亲自运行 Python 脚本对所有摘要进行文本分析与结构化提取，生成包含以下核心字段的 PICO 数据库：
...
3. **全库摘要缺失自动自愈补全矩阵 (Abstract Coverage Healing Matrix)**：
   - 数据处理 Python 脚本必须校验所有数据库条目的摘要完整度。若发现某些文献摘要缺失，自动触发 **Tier 1 摘要自愈补全**：
     ...
     - **补全途径 C (全浏览器 DOM 抓取)**：使用 `browser_*` 导航至文献 DOI / 期刊页面，直接提取 `.abstract` / `#abstract` / `meta[name="description"]` 节点内容。
...
1. **浏览器操控第一原则 (Primary Browser Automation Directive)**：
   - 100% 完全支持并优先采用 **DeepSeek++ 全浏览器自动化操控 (`browser_*` / `browser_subagent`)** 实现所有数据库（PubMed, Embase, WoS, Cochrane Library, Scopus, Google Scholar, Europe PMC, ScienceDirect, IEEE Xplore, OpenAlex, ClinicalTrials, arXiv）的搜索、摘要解析与全文 PDF 下载。
...
2. **目标数据库全浏览器操控路线图 (Full Browser Operation Roadmap)**：
   ...
   - 🌐 **Web of Science** (`https://www.webofscience.com`)：通过 `browser_*` 导航至 WoS，继承机构 EZproxy/WebVPN 认证，输入 `TS=(...)` 表达式，逐页提取 WoS Accession ID、Times Cited、DOI 与完整摘要。
   - 🌐 **Embase** (`https://www.embase.com`)：结合 `embase-session` / `embase-web-search` 与 `browser_*` 自动操控导航并提交 Emtree 表达式，抓取完整文献卡片与完整摘要。
   - 🌐 **Scopus** (`https://www.scopus.com`)：通过 `browser_*` 打开 Scopus 检索页，提取 TITLE-ABS-KEY 结果、EID/DOI 与完整摘要。
   - 🌐 **Google Scholar** (`https://scholar.google.com`)：通过 `browser_*` 模拟真实人类操作（带 3-5 秒 Jitter 停顿），输入关键词检索，提取引用次数、Snippet/Abstract 与 PDF 直链。
   - 🌐 **ScienceDirect / IEEE Xplore / ClinicalTrials / arXiv**：操控浏览器导航至目标页面，提取全部结构化条目与完整摘要。
...
- **Tier 1 (代码语法、依赖包缺失与 JSON/数据格式解析报错/摘要缺失)**：自动抓取 Traceback，调用 `pip` / `install.packages()` 补全包；遇到 `JSONDecodeError`/`KeyError`/`摘要缺失` 自动切换为容错解析与全库摘要补全，修正代码后自动重试，**严禁因解析报错或摘要缺失放弃代码运行**。
...
1. **全浏览器操控精准多库联合检索与 100% 摘要抓取**：使用 `browser_*` 顺序检索 PubMed, Embase, Web of Science, Cochrane Library, Scopus, Europe PMC, OpenAlex, Google Scholar 等全部目标数据库，100% 抓取完整摘要。
