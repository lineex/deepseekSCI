...
DeepSeekSCI 是一个可移植的 `SKILL.md` 医学科研 Agent，覆盖研究思路发掘、可行性验证、医学文献检索、研究设计、Python 分析、论文写作、内部审稿、选刊与投稿准备。
...
GitHub 输入框识别的是具体 `SKILL.md` 文件地址，不是仓库首页地址。安装时会同时取得同目录下的 `references/`、`scripts/` 和 `assets/`。
...
标准技能目录是 [`skills/deepseek-sci`](skills/deepseek-sci)。支持 Agent Skills/Claude Skills 目录协议的软件可导入该目录；只支持单提示文件的软件可读取其中的 `SKILL.md`，但完整运行仍建议保留配套的参考文件、Python 脚本和模板。
