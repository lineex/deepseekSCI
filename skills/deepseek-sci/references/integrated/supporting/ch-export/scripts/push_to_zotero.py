#!/usr/bin/env python3
"""Push Cochrane Library citation data to Zotero via local Connector API (localhost:23119).
...
def build_zotero_item(paper):
    """Build Zotero journalArticle item from Cochrane Review data."""
    ...
    item = {
        ...
        "publicationTitle": paper.get("journal", "Cochrane Database of Systematic Reviews"),
        "libraryCatalog": "Cochrane Library",
        "accessDate": now,
        "creators": [
    ...
    if paper.get("reviewType"):
        note = item.get("extra", "")
        item["extra"] = f"{note}\nreviewType: {paper['reviewType']}".strip()

    return item


def download_pdf(pdf_url, cookies="", referer="https://www.cochranelibrary.com"):
    """Download PDF from Cochrane Library using provided cookies.
...
def main():
    parser = argparse.ArgumentParser(
        description="Push Cochrane Library citations to Zotero"
    )
